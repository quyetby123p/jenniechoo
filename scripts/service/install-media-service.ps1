param(
    [string]$ServiceName = "FBMediaResearchBot",
    [string]$ProjectRoot = "",
    [string]$PythonExe = ""
)
...
$runner = Join-Path $PSScriptRoot "run-media-bot.ps1"
if (-not (Test-Path $runner)) {
    throw "Khong tim thay script run-media-bot.ps1"
}
...
sc.exe description $ServiceName "Telegram media research bot" | Out-Null
